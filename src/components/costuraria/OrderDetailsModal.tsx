          ...
          <div className="pt-6 mt-6 border-t border-gray-200 flex flex-col gap-4">
            ...
            {order.status === 'pronto' && (
              order.price - localAmountPaid > 0 ? (
                ...
                <div className="space-y-3 bg-emerald-50 p-5 rounded-2xl border border-emerald-200">
                  <p className="text-emerald-900 font-bold text-[16px] mb-2">Pedido totalmente pago! Selecione a forma de pagamento utilizada para confirmar a entrega:</p>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { id: 'dinheiro', label: 'Dinheiro' },
                      { id: 'pix', label: 'Pix' },
                      ...
                      <button
                        ...
                        disabled={isUpdating}
                        onClick={() => handleDeliverAndPayAll(method.id as any)}
                        className="h-12 rounded-xl bg-emerald-600 text-white font-bold hover:bg-emerald-700 transition-colors text-sm focus-ring"
                      >
                        {method.label}
                      </button>
                    ))}
