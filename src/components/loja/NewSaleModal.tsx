...
export function NewSaleModal({ isOpen, onClose, onSuccess }: NewSaleModalProps) {
  ...
  // Atualizar os valores da divisão quando o total muda
  useEffect(() => {
    if (isSplitPayment) {
      if (paymentAmount1 > total) {
        setPaymentAmount1(total)
        setPaymentAmount2(0)
      } else {
        setPaymentAmount2(total - paymentAmount1)
      }
    }
  }, [total, paymentAmount1, isSplitPayment])
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="space-y-6">
                  ...
                  {/* Forma 2 (Apenas se dividido) */}
                  {isSplitPayment && (
                    <div className="space-y-3 pt-4 border-t border-gray-200">
                      <div className="flex items-center justify-between">
                        <p className="font-semibold text-gray-700">2ª Forma de Pagamento:</p>
                        <p className="font-bold text-purple-900">Restante: {formatCurrency(paymentAmount2)}</p>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {paymentMethods.map(method => {
                          if (method.id === paymentMethod) return null // não permite a mesma forma 2x
                          const Icon = method.icon
                          const isSelected = paymentMethod2 === method.id
                          return (
                            <button
                              key={method.id}
